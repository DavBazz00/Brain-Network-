function [t_sol, p_sol, pt_sol] = HeterodimerInfection_treatment_aging(...
    A, CoordTable, k0, k1, ktilde1, k12, diffusion_coeff, edge_reduction, ...
    dt1, dt2, t_switch, t_end)
% HETERODIMERINFECTION_TREATMENT_AGING
%   Simula il modello eterodimero in due fasi:
%     Fase 1: aging puro con passo dt1 fino a t = t_switch
%     Fase 2: trattamento + aging con passo dt2 da t = t_switch a t = t_end

    N = size(A,1);

    % condizioni iniziali di default
    p0_init   = ones(N,1) * (k0/k1);
    pt0_init  = zeros(N,1);
    infected_mask = strcmp(CoordTable{:,4},'Entorhinal');
    pt0_init(infected_mask) = 0.1;
    ...
    [t1a, p1a, pt1a] = HeterodimerInfection_dynamic( ...
        A, CoordTable, k0, k1, ktilde1, k12, diffusion_coeff, ...
        edge_reduction, dt1, num_steps1, p0_init, pt0_init);
    t1a = t1a(:);
    ...
    for step = 2:(num_steps2+1)
        % 1.1) dynamic edge variation (treatment)
        ...
        if E>0
            ...
            if X>0
                ...
                for m = 1:numel(topE)
                    u = iList(topE(m)); v = jList(topE(m));
                    A(u,v) = A(u,v) * edge_reduction;
                    A(v,u) = A(v,u) * edge_reduction;
                end
            end
        end
        % 1.2) dynamic edge variation (aging)
        ...
        if maxEdgesToModify < 2
            ...
            if mod(maxEdgesToModify, 2) == 1, maxEven = maxEdgesToModify-1; else maxEven = maxEdgesToModify; end
            numEdgesToModify = 2 * randi([1, maxEven/2]);
        end
        if numEdgesToModify > 0
            selected = randperm(numEdgesTotal, numEdgesToModify);
            for k = 1:2:numEdgesToModify
                idx1 = edgeI(selected(k)); idx2 = edgeJ(selected(k+1));
                A(idx1,idx2)=A(idx1,idx2)*edge_reduction;
                A(idx2,idx1)=A(idx2,idx1)*edge_reduction;
            end
        end

        % 2) stochastic conversion reduction
        k12_eff = k12_eff * (rand()*0.1 + 0.9);

        % 3) Forward Euler
        L      = diag(sum(A,2)) - A;
        p_old  = p2(step-1, :)';  pt_old = pt2(step-1, :)';
